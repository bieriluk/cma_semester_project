...
packages <- c("ggplot2", "dplyr", "tidyr", "readr", "zoo", "data.table", "sf", "terra", "tmap", "stats", "randomForest", "lubridate", "trajr", "gstat", "geosphere")
...
Remove segments with a duration of less than 60s
```{r}

```


Subsegmentation by speed & Feature extraction
...
posmo_filter_segm <- posmo_filter_segm |> 
  ...
  long = st_coordinates(st_transform(posmo_filter_segm, 4326))[,1], 
  lat = st_coordinates(st_transform(posmo_filter_segm, 4326))[,2],
  bearing = atan2((cos(lat)*sin(lead(lat)) - sin(lat)*cos(lead(lat))* cos(long - lead(long))), (cos(lead(lat))* sin(long - lead(long))))
  )

```


Feature extraction by subsegment
```{r}
posmo_filter_segm_summary <- posmo_filter_segm |> 
  group_by(segment_id)|>
  summarise(
    avgspeed_ms = mean(speed_ms, na.rm = TRUE),
    avgacc_mss = mean(Y, na.rm = TRUE)
  ) |>
  ungroup()|>
```
...
```{r}
select(posmo_filter_segm, datetime, geometry, timelag_s)
