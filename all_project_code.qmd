...
packages <- c("ggplot2", "dplyr", "tidyr", "readr", "zoo", "data.table", "sf", "terra", "tmap", "stats", "randomForest", "lubridate", "trajr")
...
Look for negative timelag values and visually validate that they are corrective algorith errors, then eliminate these rows incl. NA's. We deceided to also remove rows with 0 timelag since they mess with grouping an calculations. --> In the future for improvment: leave one row if timespamps are identical!
```{r}
posmo_filter |>
    filter(timelag_s < 0)
...
tm_shape(slice(posmo_filter,1261)) +
  tm_dots()
  tm_basemap("Esri.WorldImagery")
...
```{r}
mean(posmo_filter$speed_ms, na.rm = TRUE)
median(posmo_filter$speed_ms, na.rm = TRUE)
min(posmo_filter$speed_ms, na.rm = TRUE)
max(posmo_filter$speed_ms, na.rm = TRUE)

posmo_filter |>
    filter(speed_ms > 140)

tm_shape(slice(posmo_filter,1258:1260)) +
  tm_dots()
  tm_basemap("Esri.WorldImagery")
```
...
posmo_filter_resamp <-  posmo_filter_resamp |>
#  as.data.frame(col.names = "datetime")  |>
  mutate(
    datetime = seq.POSIXt(from = min(posmo_filter$datetime, na.rm = TRUE), to = max(posmo_filter$datetime, na.rm = TRUE), by = 10),
    user_id = posmo_filter$user_id[1]
  )

posmo_filter_resamp <- posmo_filter |>
  select(user_id, datetime, X, Y) 
