...
packages <- c("ggplot2", "dplyr", "tidyr", "readr", "zoo", "data.table", "sf", "terra", "tmap", "stats", "randomForest", "lubridate", "trajr", "gstat", "geosphere", "nngeo", "vegan")
...
#         bearing_geosp = bearing(st_coordinates(st_transform(posmo_filter_segm_clean, 4326)), lead(st_coordinates(st_transform(posmo_filter_segm_clean, 4326)),1)),
         azi = c(st_azimuth(st_geometry(posmo_filter_segm_clean[-nrow(posmo_filter_segm_clean),]), st_geometry(lead(posmo_filter_segm_clean,1)[-nrow(lead(posmo_filter_segm_clean,1)),])), NA)
  )
...
seg_sinuos = TrajSinuosity2(TrajFromCoords(posmo_filter_segm_clean))
...
Preliminary multi criteria analysis to eliminate walking segments
...
```{r}
posmo_filter_segm_walks <- posmo_filter_segm_sry |> 
  mutate(
...
tm_shape(posmo_filter_seg_cluster) +
  tm_dots(col = "kmeans4") +
  tm_basemap("Esri.WorldImagery")

```
**Wie kriege ich multiple clusters bei kmeans?**
